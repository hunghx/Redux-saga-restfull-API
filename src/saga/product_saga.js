...
export function* get_product_by_status() {
  try {
    let response = yield call(product_service.get_products_by_status);

    yield put(get_product_by_status_success(response.data));
  } catch (error) {
    console.log(error);
  }
}
